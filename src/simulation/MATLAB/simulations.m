...
for t = 1:msim
    ...
    %  Method 2: Equal distance confidence bands using normalized vectors

    weights2 = ones(size(xgol,1),1).*normxl;
    xw2 = xmatnorml'.*(weights2*ones(1,size(K,1)));
    c2 = quantile(max(abs(xw2*Z'))',cr);

    c2w = c2*(weights2).^(-1);

    % Adjust (reverse normalization) - Equal distance band is xmat'beta +/- c2w
    c2w = c2w.*normxl;
    ...
    %Minimum weighted area squared

    % Weights 
    oweights = betapdf(xgo,2,2);
    % Endpoints get 0 weight so can be set to +/- inf
    xmin = fmincon(@(param)obj2(param,sx,normx,oweights), lB(2:end-1,1), [], [], [], [], [], [], @(param)constr2(param, xmatnorm, Zs, sx,cr,hn,ndraws), options);

    % Adjust to make comparable to sup-t
    weights4 = ([-1000;xmin(1:size(xgo,1)-2,1);-1000]).^(-1);
    xw4 = xmatnorm'.*(weights4*ones(1,size(K,1)));
    c4 = quantile(max(abs(xw4*Z'))',cr);

    c4ws = -[-1000;xmin(1:size(xgo,1)-2,1);-1000].*normx*c4;
    
    % Final projection to fill in grid points
    for k=1:ngl+1
        [~,c4w(k,1)] = linprog(xmatl(k,:)',[xmat;-xmat],[c4ws;c4ws]);
    end
    c4w = -c4w;
    
    % Store summary
    
    center = abs(xmatl*(beta-betah));
    
    pc1w = min(center <= c1w);
    pc2w = min(center <= c2w);
    pmin1 = min(center <= c3w); 
    pmin2 = min(center <= c4w); 
    ppoint = min(center <= norminv(1-(1-cr)/2,0,1)*sqrt(diag(xmatl*K*xmatl')));

    % coverages
    covp(t,:) = [ppoint pc1w pc2w pmin1 pmin2];
        
    output_mat(:,:,t) = [ center c1w c2w c3w c4w ];
...
function [f,df] = obj2(param,sx, normx, oweights)

    lB = param(1:sx-2,1);
    uB = -lB;
 
    f = mean( (uB - lB).*normx(2:end-1).*oweights(2:end-1));

    df = -2.*normx(2:end-1).*oweights(2:end-1)/sx;
end
...
function [c, ceq, dc, dceq] = constr2(param, xmatnorm, Zs, sx,cr,hn, ndraws)

    lB = [-1000;param(1:sx-2,1);-1000];
    uB = -lB;
    ...
    c2 = 0.1 - (uB(2:end-1) - lB(2:end-1));
    dc2 = 2*eye(sx-2);
    
	c = [c1;c2];
    dc = [dc1(2:end-1,1) dc2];
