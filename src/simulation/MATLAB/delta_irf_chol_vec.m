...
for r=1:mc_runs
    ...
    for h = 1:H
        for m = 0:(h-1)
            A_pow = (A(:,:,r)')^(h - 1 - m);
            Jacobian_Phi_1_H(:, :, h) = Jacobian_Phi_1_H(:, :, h) + kron(A_pow(1:K, :), Phi(:, :, m + 1,r));
        end
    end

    Jacobian_Phi = reshape(permute(Jacobian_Phi_1_H, [2, 1, 3]), len_vecA, len_vecirf*H)';
    Sigma_Phi(:,:,r) = Jacobian_Phi * Sigma_A_irf(:,:,r) * Jacobian_Phi';

    Jacobian_Theta_dA = cat(3,zeros(len_vecirf,len_vecA), ...
    pagemtimes(kron(P(:,:,r)',eye(K)).*ones(len_vecirf,len_vecirf,H),Jacobian_Phi_1_H));

    Jacobian_Theta_dSigma = zeros(len_vecirf,len_vechSigma,H+1);

    H_mat = L' / (L * (eye(K^2) + K_mat) * kron(P(:,:,r), eye(K)) * L');

    for h=1:H+1
        Jacobian_Theta_dSigma(:,:,h) = kron(eye(K),Phi(:,:,h,r))*H_mat;
    end

    Jacobian_Theta = reshape(permute(cat(2,Jacobian_Theta_dA,Jacobian_Theta_dSigma),[2 1 3]), ...
    len_vecA+len_vechSigma,len_vecirf*(H+1))';

    Sigma_Theta(:,:,r) = Jacobian_Theta * Omega_irf(:,:,r) * Jacobian_Theta';
